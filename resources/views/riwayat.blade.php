                    ...
                    @foreach($reservasis as $rs)
                        <div class="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 hover:border-blue-200 transition">
                            <div class="flex justify-between items-start mb-4">
                                ...
                                @elseif(stripos($rs->status, 'Selesai') !== false || stripos($rs->status, 'Lunas') !== false)
                    ...
                    @foreach($riwayatZis as $zis)
                        <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 flex flex-col h-full hover:-translate-y-1 transition duration-300">
                            <div class="flex items-center gap-3 mb-4">
                                ...
                                <div>
                                    <h3 class="font-bold text-slate-800">{{ $zis->jenis }}</h3>
                                    <p class="text-xs text-slate-500">{{ \Carbon\Carbon::parse($zis->tanggal)->format('d F Y') }}</p>
                                </div>
                            </div>
                            
                            <div class="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between">
                                <span class="text-lg font-extrabold text-slate-900">Rp {{ number_format($zis->nominal, 0, ',', '.') }}</span>
                                
                                @if($zis->status === 'Berhasil')
                                    <span class="text-xs font-bold text-green-600 bg-green-50 px-2.5 py-1 rounded-md"><i class="fa-solid fa-check mr-1"></i> Berhasil</span>
                                @else
                                    <span class="text-xs font-bold text-orange-600 bg-orange-50 px-2.5 py-1 rounded-md"><i class="fa-solid fa-clock mr-1"></i> Proses</span>
                                @endif
                            </div>
                        </div>
                    @endforeach
