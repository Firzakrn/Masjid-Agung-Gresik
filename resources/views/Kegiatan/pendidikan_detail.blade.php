...
<head>
    ...
    <title>{{ $pendidikan->judul }} - Masjid Agung Gresik</title>
...
<body class="bg-gray-50 flex flex-col min-h-screen">
    ...
    <main class="flex-grow max-w-7xl mx-auto px-4 py-12 w-full">
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            {{-- Konten Utama --}}
            <div class="flex-1">
                <h1 class="text-3xl font-bold text-green-800 mb-4">{{ $pendidikan->judul }}</h1>
                <p class="text-sm text-gray-400 mb-6">
                    <i class="fa fa-calendar mr-1"></i>
                    {{ $pendidikan->created_at->format('d F Y') }}
                </p>

                @if($pendidikan->foto)
                <img src="{{ asset('images/berita/' . $pendidikan->foto) }}"
                    class="w-full max-h-[500px] object-cover rounded-xl mb-6 shadow"
                    alt="{{ $pendidikan->judul }}">
                @endif

                <div class="prose max-w-none text-gray-700 leading-relaxed">
                    {!! nl2br(e($pendidikan->isi_konten)) !!}
                </div>
            </div>
            ...
            <div class="w-full lg:w-72 shrink-0">
                ...
                <div class="flex flex-col gap-4">
                    @foreach($pendidikanTerbaru as $item)
